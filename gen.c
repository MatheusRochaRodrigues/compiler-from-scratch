...
void gen_data_section(void)
{
    ...
    while(aux != NULL){
        ...
        for (i = 0; i < n; i++)
        {
            fprintf(output_file, "%s: ", aux->variable[i].name);

            switch (aux->variable[i].type)
            { // Por enquanto gera endereco zero
            case INT:
                fprintf(output_file, "dd \"%%d\", 4\n");
                break;
...
void gen_label_FuncName( char *name ) {
    ...
    sprintf(conv_value, "%d", nlabels);
    strcpy(str_name, "func");
    strcat(str_name, conv_value);
    strcpy(name, str_name);
    nlabels++;
}

// void gen_label_Function(char *label)
// {
...
void gen_assignment(char *lexema, int type)
{
    switch (type)
    {
    case INT:
        fprintf(output_file, "mov rax, 4\n");
        fprintf(output_file, "pop rax\n");

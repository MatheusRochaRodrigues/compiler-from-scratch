...
type_symbol_table_func global_symbol_table_Function;
...
type_symbol_function_entry *sym_func_find(char *s, type_symbol_table_func *stv) {
	int i;
...
type_symbol_function_entry *sym_func_declare(char *name, int type, int nparams, int params[MAX_PARAMS], char label[MAX_CHAR], type_symbol_table_func *stv) {
	if (stv->n_variables < MAX_SYMBOLS) { //Verifica se eh possivel declarar mais variavel
		int n_var;
        n_var = stv->n_variables;
        stv->variable[n_var].type = type;
        stv->variable[n_var].nparams = nparams;
        stv->variable[n_var].flag = 0;
        for (int i = 0; i < nparams; i++){
            stv->variable[n_var].params[i] = params[i];
        }
        strcpy(stv->variable[n_var].name, name);
        
        strcpy(stv->variable[n_var].label, label);
        ...
        printf("[ERRO] Limite de declaracao (quantidade) de parametros atingido.\n");
        return NULL;
    }
}
